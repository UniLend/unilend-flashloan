...
const CurrencySelectModel: FC<Props> = ({ handleClose, currFieldName, handleCurrChange, activeTab }) => {
  ...
  const Row = useCallback(({ data, index, style }) => {
    ...
    return (
      <button className="list-group-item custom-list-group" style={style} onClick={() => handleCurrChange(currency)}>
        <div className="row list-row">
          ...
          <div className="col-7">
            ...
            <div className="row">
              <p className="mb-0 list-desc" style={{ textTransform: 'capitalize' }}>
                {currency.name}
              </p>
              {currency.isCustomToken && <div className="list-user-add">Added by user</div>}
            </div>
          </div>
          <div className="col-3" style={{ alignSelf: 'center' }}>
            ...
            <div className="row bal-price">
              <p className="mb-0 list-desc" style={{ textTransform: 'capitalize' }}>
                {currency.underlyingBalance >= 0 ? currency.underlyingBalance : ''}
              </p>
            </div>
          </div>
        </div>
      ...
      {searchedToken && (
        <div style={{ padding: '0 10px' }}>
          <SearchTokenCard
            ...
            loading={searchedToken.isRequesting}
            isExist={isExist}
          />
        </div>
      )}
            ...
            <div className={`title ${openManage ? 'manage' : 'token'}`}>
              {openManage && (
                <span className={`arrow-btn ${theme}`}>
                  <i className="fa fa-arrow-left" aria-hidden="true" onClick={() => setOpenManage(false)} />
                </span>
              )}
              <span className="form-label">{openManage ? 'Manage' : 'Select a token'}</span>
